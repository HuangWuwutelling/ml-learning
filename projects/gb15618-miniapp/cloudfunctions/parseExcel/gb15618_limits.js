/**
 * GB 15618-2018 农用地土壤污染风险管控标准 限值表（CommonJS）
 *
 ...
 *
 * 部署注意事项：
 *   本文件是 CommonJS 格式（云函数 Node 12 不支持 ESM）。
 *   项目根的 data/gb15618_limits.js 是规范源；每个云函数目录内需要一份同步副本。
 */

const METALS = ['Cd', 'Hg', 'As', 'Pb', 'Cr', 'Cu', 'Zn', 'Ni'];
const CROPS = ['水田', '旱地', '果园'];
const PH_BOUNDS = [5.5, 6.5, 7.5];
...
function getLimits(crop, pH, metal) {
...
function evaluate(crop, pH, metals) {
  const details = [];
  let maxPi = 0;
  let maxPiControl = 0;
...
module.exports = { METALS, CROPS, PH_BOUNDS, LIMITS, pHIndex, getLimits, evaluate };