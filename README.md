...
| 为什么 ChatGPT 逐 token 输出：LLM 流式输出的本质与拼装（已发 2026-09-13） | SSE, FastAPI streaming, EventSource, transformers TextIteratorStreamer, TTFT, TPOT |
| 为什么 ChatGPT 会被骗：6 层防线抵御 Prompt Injection（已发 2026-09-14） | 输入预处理, 攻击检测, Prompt 隔离, System 加固, 输出验证, 行动沙盒, Qwen2.5-0.5B |
